...
const reviews = [
  { name: "Anna K.",    text: "To było niesamowite doświadczenie. Relaks na najwyższym poziomie, jak w luksusowym spa.", rating: 5 },
  { name: "Marcin W.",  text: "Magia dotyku i cudowna atmosfera. Ciało i dusza naprawdę odpoczęły.", rating: 5 },
  { name: "Ewelina Z.", text: "Wspaniałe podejście, spokój i najwyższa jakość. Czuję się odmieniona!", rating: 5 },
  { name: "Karol N.",   text: "Relaks totalny. Nigdzie wcześniej nie czułem się tak комфортowo.", rating: 5 },
  { name: "Joanna M.",  text: "Dotyk profesjonalizmu. Polecam każdemu, kto chce naprawdę odpocząć.", rating: 5 },
];
...
function PremiumStars({ rating = 5, className = "" }) {
  return (
    <div className={`flex items-center gap-1 ${className}`} aria-label={`Ocena ${rating} na 5`}>
      {Array.from({ length: rating }).map((_, i) => (
        <AiFillStar
          key={i}
          className="w-4 h-4"
          style={{ color: GOLD, filter: "drop-shadow(0 0 6px rgba(214,177,106,.25))" }}
        />
      ))}
    </div>
  );
...
function ReviewCard({ name, text, rating }) {
  return (
    <motion.figure
      ...
      style={{ backgroundImage: `linear-gradient(120deg, ${GOLD}99, transparent 35%, ${GOLD}22)` }}
    >
      <div className="relative rounded-2xl h-full bg-secondaryColor/60 backdrop-blur-lg px-6 py-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.07),0_10px_30px_rgba(0,0,0,0.4)] flex flex-col">
        <div className="pointer-events-none absolute -top-3 left-6 text-5xl leading-none select-none" style={{ color: `${GOLD}66` }}>“</div>
        ...
        <div className="flex items-center gap-4 mb-3">
          <div className="relative">
            ...
            <span className="absolute inset-0 rounded-full pointer-events-none" style={{ boxShadow: `inset 0 0 0 1px ${GOLD}55` }} />
          </div>

          <div className="flex flex-col">
            <span className="text-sm tracking-[0.12em] uppercase text-white font-medium">{name}</span>
...
export default function ReviewsSection() {
  return (
    ...
    >
      <div className="pointer-events-none absolute inset-0" style={{ boxShadow: "inset 0 0 200px rgba(0,0,0,0.8)" }} />
        ...
        <ScrollAnimationWrapper>
          ...
          <div className="text-center pb-8">
            <div className="inline-flex flex-wrap items-center justify-center gap-x-3 gap-y-1 px-5 py-2 rounded-full bg-white/5 text-white/90 backdrop-blur-sm shadow-[inset_0_1px_0_rgba(255,255,255,0.06)]">
              ...
              <span className="text-sm tracking-[0.18em] uppercase">5.0 / 5 na Booksy</span>
